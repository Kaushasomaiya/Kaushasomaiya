...
namespace Test_Taste_Console_Application.Domain.Services
{
    ...
    public class PlanetService : IPlanetService
    {
        ...
        {
            Logger.Instance.Info($"{LoggerMessage.APICallStart}");
            ...
            // Calculated average moon grevity
            allPlanetsWithTheirMoons.Where(w => w.Moons.Count() > 0).ToList().ForEach(w => w.AverageMoonGravity = w.Moons.Average(k => k.Gravity));
            Logger.Instance.Info($"{LoggerMessage.APICallEnd}");
            return allPlanetsWithTheirMoons;
